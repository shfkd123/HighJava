...
/*
 	...
 	어노테이션 타입 정의하기
 	@interface 어노테이션이름 {
 		타입요소이름(); //메서드는 아니고 메서드 모양처럼 => 반환값이 있고 매개변수는 없는 추상메서드 형태
 		...
 	}
...
@Target(ElementType.METHOD) //anotation이 적용가능한 대상을 지정함.  - target정보를  METHOD뒤에 붙임
@Retention(RetentionPolicy.RUNTIME) //어노테이션이 유지되는 기간(SOURCE, CLASS, RUNTIME)
//SOURCE : 소스코드(.java)까지 남아있다. (클래스파일에는 보이지 않음)
//CLASS : 클래스 파일(.class)까지 남아있다.(=바이트 코드)
//RUNTIME : 런타임까지 남아있는다. (=사실상 안 사라진다.)

public @interface PrintAnnotation { //어노테이션 인터페이스  PrintAnnotation
