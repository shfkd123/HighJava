...
public class RemoteClient {
	public static void main(String[] args) {
		...
		try {
			...
			//	   사용할 객체를 불러온다.
			Registry reg = LocateRegistry.getRegistry("192.168.43.132", 8888); //getRegistry 서버에 접속한다. 접근하고자 하는 ip
			RemoteInterface clientInf = (RemoteInterface) reg.lookup("server"); //lookup 실제로 접속하는 부분 "이름" ---> 등록한 원격객체를 가져온다.
			...
			files[0] = new File("d:/D_Other/y.jpg");
			files[1] = new File("d:/D_Other/y.jpg");
			...
			FileInputStream fis = null;
				for (int i = 0; i < files.length; i++) {
					...
					fInfo[i].setFileName(files[i].getName());  //<--읽어온 데이터를 배열에 담는다.
