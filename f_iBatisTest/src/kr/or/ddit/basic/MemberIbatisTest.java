...
public class MemberIbatisTest {
	public static void main(String[] args) {
		...
		try {
			...
			//2) SqlMapCilent 객체 변수를 이용하여 해당 쿼리문을 실행한다.
			//형식 ) smc.insert("namespace값.id값", 파라미터 객체(필요한 데이터를 가지고 있는))
			//		반환값 : 성공하면 null이 반환된다. <-> JDBC에서는 insert시 반환값이 int값이었음!
			...
			cnt = smc.delete("memberTest.deleteMember", "b001"); //memId를 알려줌.
			if(cnt > 0) {
...
/*			// 1) 응답의 결과가 여러개일 경우
			...
			//select는 메소드가 2가지로 나눠짐.
			//(result set) 결과값이 한건인 경우와 한건 이상인 경우로
