...
/**
	...
	- 세션을 통해서 사용자(웹 브라우저)별로 구분하여 정보를 관리할 수 있다. (세션ID 이용)
	  (쿠키를 사용할 때보다 보안이 향상된다. (정보가 서버에 저장되기 때문에...)
	  	 > 세션은 특정한 사용자별로 단위로 만들어진다. 
	
	- 세션객체를 가져오는 방법
	  HttpSession session = request.getSession(boolean값);
	  boolean값 : true인 경우 => 존재하면 그 세션객체를 던져주고, 세션객체가 존재하지 않으면 새로 생성함.
	  			 (선호) false인 경우 =>  존재하면 그 세션객체를 던져주고, 세션객체가 존재하지 않으면 null 리턴함.
	  			 ex) 로그인 시 
...
public class T06_ServletSessionTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		HttpSession session = req.getSession(true); //존재하면 그 세션객체를 던져주고, 세션객체가 존재하지 않으면 새로 생성함.
