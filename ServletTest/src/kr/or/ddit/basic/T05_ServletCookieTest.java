...
public class T05_ServletCookieTest extends HttpServlet{
	...
	/**
	 * Http는 비연결 지향성 프로토콜이기 때문에 사용자를 구분하기 위해서는 쿠키를 사용해야 한다. 
	 * 
	 * 쿠키 정보를 다루기 위한 예제
	 * (웹 서버와 브라우저는 애플리케이션을 사용하는 동안 필요한 값을 쿠키를 통해 공유하여 상태를 유지함.)
	 * 1. 구성 요소
	 * - 이름
	 * - 값
	 * - 유효시간(초)
	 * - 도메인: ex) www.somehost.com, .somehost.com => 쿠키의 도메인이 쿠키를 생성한 서버의 도메인을 벗어나면 브라우저는 
	 * 													쿠키를 저장(생성) 하지 않는다. (보안 때문에)
	 * 2. 동작방식
		...
		if(cookies != null) {
			...
			for (Cookie cookie : cookies) {
				if((cookie.getName()).equals("userId")) {
					//쿠기 제거하기
					cookie.setMaxAge(0); //0으로 세팅하면 삭제됨.
	 ...
	 */
		//쿠키 생성하기
		Cookie userId = new Cookie("userId", req.getParameter("userId"));  //사용자가 던져준 userId를 userId라는 이름으로 쿠키 생성
